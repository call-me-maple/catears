...
func lookForMemes(m *discordgo.MessageCreate) (err error) {
	...
	err = publishMessage(m.ChannelID, content)
	if err != nil {
		return
	}
	return
...
func respondToMention(m *discordgo.MessageCreate) error {
	...
	err := publishReaction(m.ChannelID, m.ID, emoji)
	if err != nil {
		return err
	}
	return nil
...
func findMusic(m *discordgo.MessageCreate) error {
	...
	err = publishMessage(m.ChannelID, randMusic.Content)
	if err != nil {
		return err
	}
	return nil
...
func countDown(m *discordgo.Message) (err error) {
	...
	for i := wait; i > 0; i-- {
		...
		err = publishMessage(m.ChannelID, content, delay)
		if err != nil {
			return
		}
	}
	...
	err = publishMessage(m.ChannelID, "now?", delay)
	if err != nil {
		return
	}
